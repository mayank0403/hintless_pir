...
class HintlessSimplePIRRunner:
    ...
    def parse_outputs(self, output):
        ...
        patterns = {
            'Client A Expansion (s)': r'Client expanding A from seed time: [\d.]+ ms \| ([\d.]+) sec',
        ...
        parsed_metrics_path = os.path.join(log_dir, parsed_metrics_file)
        with open(parsed_metrics_path, 'w') as f:
            f.write("Parsed Metrics:\n")
            for metric, value in metrics.items():
                f.write(f"{metric}: {value}\n")
        print(f"Parsed metrics saved to {parsed_metrics_path}")
